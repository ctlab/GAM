...
Construct a mouse-specific network with command `makeKeggNetowrk`, but first load 
KEGG reaction data (part of `GAM`) and mouse annotation package. See section 
[Constructing network](#constructing-network) for details.
...
Make differential expressions analysis between M1 and M2 polarizations. The `GAM` package
contains helper functions do make it in one command, but you can do it manually, see documentation
...
for CPLEX. See section [MWCS solvers](#mwcs-solvers) for additional information.
```{r}
...
```{r}
module <- addInterconnections(module.rn, es.rn)
```
...
`GAM` also can use **heuristic`fastHeinz` solver** which is part of the `BioNet` package. Its results
can be significantly worse than other solver's, but it doesn't have any external
